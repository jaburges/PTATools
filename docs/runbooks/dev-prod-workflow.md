...
- **Staging slot WordPress is currently returning HTTP 500.** The slot is running and the configuration is correct (`DATABASE_HOST` etc. all set, slot-sticky markers in place), but WP itself raises a critical error. The staging DB exists on the shared MySQL server but may not have schema initialized, or the managed-identity DB auth from the staging slot may not be configured. The CI/CD pipeline still works at the file-deploy level — `deploy-staging.yml` tolerates 500s and reports them as warnings rather than failures. Repair this before relying on staging for actual code testing. (Investigate by enabling WP_DEBUG temporarily in staging slot and checking `/home/LogFiles/`.)
