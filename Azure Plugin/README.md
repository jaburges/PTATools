...
- **`[up-next columns="N"]` is honored regardless of theme.** Earlier
  versions hard-coded the theme's `columns` value into the
  generated grid CSS, so `[up-next theme="ptsa" columns="2"]` was
  silently rendered in whatever the theme's column count was
  (often 1). Now the CSS keys grid rules on the wrapper's
  `.upcoming-columns-N` class instead, so the shortcode attribute
  is the source of truth and the theme's `columns` is just the
  fallback default. Column cap also bumped from 3 → 4 to match
  the admin UI's max.
...
- **`[up-next]` theme Preview button now actually previews.**
  Clicking Preview on any theme row in **Calendar → Upcoming
  Events → Themes** re-renders the Live Preview section below
  with `[up-next theme="<slug>" cache="false"]` via AJAX, scrolls
  it into view, and shows a banner with the exact shortcode
  (with Copy + Show default buttons). Replaces the placeholder
  message shipped in v3.125. The default-theme HTML is held
  client-side so Show default reverts instantly without a
  round-trip.
...
- **Dismiss Recent Email Errors.** The dashboard panel was sticky —
  stale "Maximum credits exceeded" / "SMTP data not accepted" rows
  from a month ago lingered with no way to clear them. Added a per-row
  `\u00d7` button (deletes the underlying log row) and a "Dismiss all"
  button (non-destructive: sets `azure_email_errors_dismissed_until`
  option to now so older errors no longer surface in the panel; rows
  still visible in the Logs tab for audit). Backed by new AJAX
  endpoint `azure_dismiss_email_errors`.
