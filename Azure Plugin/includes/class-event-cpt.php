...
class Azure_Event_CPT {
    ...
    /**
     * Provider slug for a meeting URL (teams, zoom, meet, …).
     *
     * @param string $url
     * @return string
     */
    public static function online_meeting_provider_slug($url) {
        ...
        if (strpos($host, 'teams.microsoft.com') !== false || strpos($host, 'teams.live.com') !== false) {
            return 'teams';
        }
        if (strpos($host, 'zoom.us') !== false || strpos($host, 'zoomgov.com') !== false) {
            return 'zoom';
        }
        if (strpos($host, 'meet.google.com') !== false) {
            return 'meet';
        }
        if (strpos($host, 'webex.com') !== false) {
            return 'webex';
        }
        if (strpos($host, 'gotomeeting.com') !== false || strpos($host, 'goto.com') !== false) {
            return 'goto';
        }
        if (strpos($host, 'skype.com') !== false) {
            return 'skype';
        }
        return 'generic';
    }
    ...
    /**
     * Monochrome SVG for an icon-only join control. currentColor so themes
     * can tint it.
     *
     * @param string $slug From online_meeting_provider_slug().
     * @return string
     */
    ...
    public static function render_join_meeting_button($post_id, $variant = 'inline') {
        return self::render_join_meeting_markup(self::extract_online_meeting_url($post_id), $variant);
    }
    ...
    public static function render_join_meeting_markup($url, $variant = 'inline') {
        ...
        if ($variant === 'icon') {
            ...
            return sprintf(
                '<a class="pta-join-meeting pta-join-meeting--icon" href="%s" target="_blank" rel="noopener noreferrer" data-provider="%s" aria-label="%s" title="%s">%s</a>',
                esc_url($url),
                esc_attr($slug),
                esc_attr($aria),
                esc_attr($aria),
                self::join_meeting_icon_svg($slug)
            );
        }
