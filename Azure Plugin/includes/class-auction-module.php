...
class Azure_Auction_Module {
    ...
    public function enqueue_auction_scripts() {
        ...
        wp_localize_script('azure-auction-bid', 'azureAuction', array(
            ...
            // Pre-computed login URL with redirect_to back to this auction,
            // so the session-expired banner has a working link even if the
            // AJAX 401 payload is stripped by an intermediary proxy.
    ...
    public function ajax_place_bid_guest() {
        // Reply HTTP 401 + structured payload so the JS can recognise the
        // logged-out case and swap the bid form for a "Session expired â
        // log in to bid" banner. Without the explicit `code:'not_logged_in'`
        // signal the front-end only sees a generic error and the bidder has
        // to figure out their session expired (the recurring "I bid but it
        // never recorded" failure mode).
    ...
    public function ajax_place_bid() {
        if (!is_user_logged_in()) {
            // Defence in depth â cookie expired mid-request between page
            // render and AJAX submit. Same payload shape as the nopriv
            // handler so the JS treats both the same way.
            $referer = isset($_SERVER['HTTP_REFERER']) ? wp_validate_redirect((string) $_SERVER['HTTP_REFERER'], '') : '';
            wp_send_json_error(array(
                'code'      => 'not_logged_in',
                'message'   => __('Your session has expired. Log in to place bids.', 'azure-plugin'),
