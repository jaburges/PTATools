...
class AzurePlugin {
    ...
    public function init() {
        ...
        try {
            ...
            $this->require_module_files(array('class-web-push.php', 'class-home-screen.php'));
            if (class_exists('Azure_Home_Screen')) {
                Azure_Home_Screen::get_instance();
            }
            ...
            // Live list of archive pages under the Newsletters parent.
            // Loaded globally so [newsletter-archive] works on any page;
            // the constructor only registers the shortcode tag.
            if (file_exists(AZURE_PLUGIN_PATH . 'includes/class-newsletter-archive-shortcode.php')) {
                require_once AZURE_PLUGIN_PATH . 'includes/class-newsletter-archive-shortcode.php';
                if (class_exists('Azure_Newsletter_Archive_Shortcode')) {
                    Azure_Newsletter_Archive_Shortcode::get_instance();
                }
            }

            // Single newsletter pages store campaign HTML as post_content.
            // Scope ChromeNews resets so leftover CTA cells and divider
            // tables do not pick up theme borders/buttons.
            if (file_exists(AZURE_PLUGIN_PATH . 'includes/class-newsletter-page.php')) {
                require_once AZURE_PLUGIN_PATH . 'includes/class-newsletter-page.php';
                if (class_exists('Azure_Newsletter_Page')) {
                    Azure_Newsletter_Page::get_instance();
                }
            }
    ...
    private function init_product_fields_components($ctx) {
        try {
            ...
            if (class_exists('Azure_Express_Checkout')) {
                Azure_Express_Checkout::get_instance();
            }
    ...
    private function init_donations_components($ctx) {
        try {
            $this->require_module_files(array('class-class-competitions.php', 'class-donations-module.php'));
        ...
        try {
            ...
            if (!get_option('azure_plugin_settings')) {
                $default_settings = array(
                    ...
                    // Event store: stay on TEC when that plugin is already
                    // running; otherwise PTA Tools is the Outlook write target.
                    'pta_calendar_owner' => (class_exists('Tribe__Events__Main') || (function_exists('post_type_exists') && post_type_exists('tribe_events'))) ? 'tec' : 'pta',
                );
